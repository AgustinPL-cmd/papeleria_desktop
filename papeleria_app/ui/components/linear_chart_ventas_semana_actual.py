...
class GraficaLinealVentas:
    def __init__(self, titulo: str, datos: list, tipo_x: str, height = 200):
        """
        :param titulo: Título de la gráfica
        :param datos: Lista de tuplas (etiqueta_x, total_ventas, total_ingresos, ganancia_neta)
    ...
    def construir_grafica(self):
        datos_dict = {str(d[0]): d for d in self.datos}
        ventas, ingresos, ganancias = [], [], []
        max_valor = 0

        for i, etiqueta in enumerate(self.ejes_x_ordenados):
            if etiqueta in datos_dict:
                _, total_v, total_i, ganancia = datos_dict[etiqueta]
            else:
                total_v, total_i, ganancia = 0, 0, 0

            ventas.append(ft.LineChartDataPoint(i, float(total_v)))
            ingresos.append(ft.LineChartDataPoint(i, float(total_i)))
            ganancias.append(ft.LineChartDataPoint(i, float(ganancia)))
            max_valor = max(max_valor, total_v, total_i, ganancia)
        ...
        return ft.LineChart(
            data_series=[
                ft.LineChartData(data_points=ventas, stroke_width=5, color="#1da3c2", curved=True, stroke_cap_round=True),
                ft.LineChartData(data_points=ingresos, stroke_width=5, color="#17b39e", curved=True, stroke_cap_round=True),
                ft.LineChartData(data_points=ganancias, stroke_width=5, color="#9561e2", curved=True, stroke_cap_round=True),
            ],
